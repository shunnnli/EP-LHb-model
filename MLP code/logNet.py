...
class TopKLinear(nn.Module):
    ...
    def __init__(
        ...
        trainable_sign=False,  # Default: sign matrix is not trainable
    ):
        ...
        self.sign_matrix = nn.Parameter(
            torch.empty((out_features, in_features)), requires_grad=trainable_sign)
    ...
    def initialize_signs(self, sign_pattern):
        ...
        if sign_pattern == 'mixed':
            ...
            raise ValueError("Invalid sign pattern. Choose 'mixed' or 'negative'.")
...
# Example Usage
# First layer: mixed signs, sign matrix is not trainable
layer1 = TopKLinear(in_features=128, out_features=64, K=10, sign_pattern="mixed", trainable_sign=False)

# Second layer: mixed signs, sign matrix is trainable
layer2 = TopKLinear(in_features=64, out_features=32, K=5, sign_pattern="mixed", trainable_sign=True)

# Final layer: purely negative signs, sign matrix is not trainable
layer3 = TopKLinear(in_features=32, out_features=10, K=3, sign_pattern="negative", trainable_sign=False)

# Combine into a sequential model
model = nn.Sequential(layer1, nn.ReLU(), layer2, nn.ReLU(), layer3)

# Verify the trainability of the sign matrices
print("Layer 1 trainable sign matrix:", layer1.sign_matrix.requires_grad)
print("Layer 2 trainable sign matrix:", layer2.sign_matrix.requires_grad)
print("Layer 3 trainable sign matrix:", layer3.sign_matrix.requires_grad)
