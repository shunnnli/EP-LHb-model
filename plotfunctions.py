...
def plotSEM(x, y, omissions=None, label=None, color=None, ax=None, alpha=0.2, fill=True, fill_alpha=0.2):
    ...
    if not fill:
        ...
        std  = np.std(y, axis=0)
        ax.plot(x, mean, color=color, label="_nolegend_")
        ax.fill_between(x, mean - std, mean + std, alpha=fill_alpha, color=color, edgecolor='None', label="_nolegend_")
...
def plotScatterBar(data,
    ...
    if style == 'box':
        ...
        bp = ax.boxplot(
            data,
            positions=x + pos_offset,
            widths=width,
            patch_artist=True,
            boxprops=dict(linewidth=1) if fill else dict(color='black', facecolor='none', linewidth=0.5, alpha=0.7),
            whiskerprops=dict(linewidth=error_bar_width) if fill else dict(color='black', linewidth=0.5, alpha=0.7),
            capprops=dict(linewidth=error_bar_width) if fill else dict(color='black', linewidth=0.5, alpha=0.7),
            medianprops=dict(linewidth=1) if fill else dict(color='black', linewidth=0.5, alpha=0.7),
            flierprops=flierprops if fill else dict(
                marker='o',
                markerfacecolor='none',
...
def load_recorder_data(recorder,
    ...
    if hasattr(recorder, "td_pid_errors"):
        td_pid_errors = np.array(recorder.td_pid_errors)[1:]
    else:
        td_pid_errors = td_errors + np.array([k * d for k, d in zip(recorder.kd, recorder.d)])[1:]
        
    
    if hasattr(recorder, "kd_d"):
        kd_d = np.array(recorder.kd_d)[1:]
    else:
        kd_d = np.array([k * d for k, d in zip(recorder.kd, recorder.d)][1:])
    ...
    cue_kd_d    = get_traces(kd_d, tones, pre_steps, post_steps)
    ...
    if not isinstance(cue_kd_d, np.ndarray):
        cue_kd_d = np.array(cue_kd_d)
    ...
    return (num_trials, reward_history, p_kp_history, i_ki_history, d_kd_history, kp_history, ki_history, 
            kd_history, cue_licks, cue_error, cue_pid_error, cue_kd_d, cue_omissions, cue_levels, trial_anticipatory_licks, 
            trial_TD_amplitude, trial_pid_TD_amplitude, t_axis, trial_axis, error)
...
def plot_figure(recorder,
    ...
    ax1.plot(trial_axis,kp_history, label='p*kp', color='tab:blue', alpha=0.7)
    ax1.plot(trial_axis,ki_history, label='i*ki', color='tab:orange', alpha=0.7)
    ax1.plot(trial_axis,kd_history, label='d*kd', color='tab:red', alpha=0.7)
    ...
    plotSEM(t_axis, cue_error, omissions=None, label="TD Error", color='tab:blue', ax=ax3, alpha=0.05, fill=True) 
    plotSEM(t_axis, cue_pid_error, omissions=None, label="PID TD Error", color='tab:orange', ax=ax3, alpha=0.05, fill=True)
    plotSEM(t_axis, cue_kd_d, omissions=None, label="Kd * D", color='tab:green', ax=ax3, alpha=0.05, fill=True)
    ...
    ax3.plot([], [], color='tab:orange', label="PID TD Error", linewidth=1)
    ax3.plot([], [], color='tab:green', label="Kd * D", linewidth=1)
